...
static void
print_flowspec_list_payload(struct flowspec *f, int type, int is_v6)
{
	const uint8_t *comp;
	uint16_t offset;
	int complen, contlen, partlen, off = 0;
	...
	while (complen > 0)
	{
		...
		printf("ptype %u ", comp[FLOWSPEC_TYPE_PAYLD_TYPE_OFFSET] >> 4);
		printf("otype %u ", comp[FLOWSPEC_TYPE_PAYLD_TYPE_OFFSET] & 0xf);
	
		offset = ((uint16_t)comp[FLOWSPEC_TYPE_PAYLD_OFFSET_OFFSET] << 8) | (uint16_t)comp[FLOWSPEC_TYPE_PAYLD_OFFSET_OFFSET + 1];
		printf("offset %u ", offset);

		contlen = comp[FLOWSPEC_TYPE_PAYLD_CONTL_OFFSET];
		printf("length %u ", contlen);

		printf("content 0x");
		for (int i = 0; i < contlen; i++)
			printf("%02X", comp[FLOWSPEC_TYPE_PAYLD_HL + i]);
		printf(" ");

		printf("mask 0x");
		for (int i = 0; i < contlen; i++)
			printf("%02X", comp[FLOWSPEC_TYPE_PAYLD_HL + contlen + i]);

		partlen = FLOWSPEC_TYPE_PAYLD_HL + 2 * contlen;
		off += partlen;
		complen -= partlen;
	}
...
show_flowspec(struct flowspec *f)
{
	...
	print_flowspec_list_payload(f, FLOWSPEC_TYPE_PAYLOAD, is_v6);
