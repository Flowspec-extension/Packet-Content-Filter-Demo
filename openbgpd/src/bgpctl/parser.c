...
static const struct token t_flowpayload_ptype[];
static const struct token t_flowpayload_otype[];
static const struct token t_flowpayload_offset[];
static const struct token t_flowpayload_ctlen[];
static const struct token t_flowpayload_content[];
static const struct token t_flowpayload_ctmask[];
static const struct token t_flowpayload_and[];
...
static const struct token t_flowrule[] = {
	...
	{ FLOW_PAYLOAD,	"payload",		NONE,		t_flowpayload_ptype},
	{ ENDTOKEN,	"",		NONE,		NULL}
};
...
static const struct token t_flowpayload_ptype[] = {
	{ FLOW_PAYLD_PTYPE,	"ptype",		NONE,		t_flowpayload_otype},
	{ FLOW_PAYLD_PTYPE,	"",		NONE,		t_flowpayload_otype},
	{ ENDTOKEN,	"",		NONE,		NULL}
};

static const struct token t_flowpayload_otype[] = {
	{ FLOW_PAYLD_OTYPE,	"otype",		NONE,		t_flowpayload_offset},
	{ FLOW_PAYLD_OTYPE,	"",		NONE,		t_flowpayload_offset},
	{ ENDTOKEN,	"",		NONE,		NULL}
};

static const struct token t_flowpayload_offset[] = {
	{ FLOW_PAYLD_OFFSET,	"offset",		NONE,		t_flowpayload_ctlen},
	{ FLOW_PAYLD_OFFSET,	"",		NONE,		t_flowpayload_ctlen},
	{ ENDTOKEN,	"",		NONE,		NULL}
};

static const struct token t_flowpayload_ctlen[] = {
	{ FLOW_PAYLD_CTLEN,	"length",		NONE,		t_flowpayload_content},
	{ FLOW_PAYLD_CTLEN,	"",		NONE,		t_flowpayload_content},
	{ ENDTOKEN,	"",		NONE,		NULL}
};

static const struct token t_flowpayload_content[] = {
	{ FLOW_PAYLD_CONTENT,	"content",		NONE,		t_flowpayload_ctmask},
	{ FLOW_PAYLD_CONTENT,	"",		NONE,		t_flowpayload_ctmask},
	{ ENDTOKEN,	"",		NONE,		NULL}
};

static const struct token t_flowpayload_ctmask[] = {
	{ FLOW_PAYLD_CTMASK,	"mask",		NONE,		t_flowpayload_and},
	{ FLOW_PAYLD_CTMASK,	"",		NONE,		t_flowpayload_and},
	{ ENDTOKEN,	"",		NONE,		NULL}
};

static const struct token t_flowpayload_and[] = {
	{ FLOW_PAYLD_AND,	"and",		NONE,		t_flowpayload_ptype},
	{ ANYTOKEN,	"",		NONE,		t_flowrule},
	{ ENDTOKEN,	"",		NONE,		NULL}
};
...
int	parse_flow_numstr(const char *, struct parse_result *, enum token_type);
	...
	*argsused = 1;
	...
	for (i = 0; table[i].type != ENDTOKEN; i++) {
		...
		case FLOW_PAYLD_AND:
			...
			break;
		case FLOW_PAYLD_PTYPE:
		case FLOW_PAYLD_OTYPE:
		case FLOW_PAYLD_OFFSET:
		case FLOW_PAYLD_CTLEN:
		case FLOW_PAYLD_CONTENT:
		case FLOW_PAYLD_CTMASK:
			if (word == NULL || wordlen == 0)
				break;
			if (strlen(table[i].keyword) == 0) {
				parse_flow_numstr(word, &res, table[i].type);
				match++;
				t = &table[i];
			} else if (strncmp(word, table[i].keyword, wordlen) == 0 && argc > 1) {
				parse_flow_numstr(argv[1], &res, table[i].type);
				*argsused += 1;
				match++;
				t = &table[i];
			}
...
static void
getcommunity_flowrate(char *s, uint32_t *val)
{
	const char	*errstr;
	float max, fval;

	max = FLT_MAX;
	fval = strtonum_float(s, 0, max, &errstr);
	if (errstr)
		errx(1, "Community %s is %s (max: %e)", s, errstr, max);
...
static void
push_numstr(const char *word, uint8_t *ptr, uint8_t contlen)
{
	if (word[0] == '0' && word[1] == 'x') {
		word += 2;
		for (int i = 0; i < contlen; i++) {
			if (word[2 * i] >= '0' && word[2 * i] <= '9')
				ptr[i] = (uint8_t)(word[2 * i] - '0');
			else if (word[2 * i] >= 'A' && word[2 * i] <= 'F')
				ptr[i] = (uint8_t)(word[2 * i] - 'A') + 10;
			else
				ptr[i] = (uint8_t)(word[2 * i] - 'a') + 10;
			...
			if (word[2 * i + 1] >= '0' && word[2 * i + 1] <= '9')
				ptr[i] += (uint8_t)(word[2 * i + 1] - '0');
			else if (word[2 * i + 1] >= 'A' && word[2 * i + 1] <= 'F')
				ptr[i] += (uint8_t)(word[2 * i + 1] - 'A') + 10;
			else
				ptr[i] += (uint8_t)(word[2 * i + 1] - 'a') + 10;
		}
	} else {
...
int
parse_flow_numstr(const char *word, struct parse_result *r, enum token_type toktype)
{
	const char *errstr;
	uint8_t *comp, *ptr;
	uint8_t complen, contlen, offset;
	long long val;
	int type;
	...
	if (r->flow.components[type] == NULL) {
		r->flow.components[type] = calloc(1, sizeof(uint8_t));
		if (r->flow.components[type] == NULL)
			err(1, NULL);
		r->flow.complen[type] = 1;
	}
	complen = r->flow.complen[type];
	comp = r->flow.components[type];
	offset = comp[0] + 1;
	ptr = comp + offset;
	...
	switch (toktype) {
		...
		case FLOW_PAYLD_CTLEN:
			if (offset == complen) {
				comp = realloc(r->flow.components[type], complen + FLOWSPEC_TYPE_PAYLD_HL);
				if (comp == NULL)
					err(1, NULL);
				complen += FLOWSPEC_TYPE_PAYLD_HL;
			}
			if (complen - offset < FLOWSPEC_TYPE_PAYLD_HL)
				return (0);

			val = strtonum(word, LLONG_MIN, LLONG_MAX, &errstr);
			if (errstr)
				errx(1, "\"payload\" invalid number: %s", errstr);

			switch (toktype) {
				case FLOW_PAYLD_PTYPE:
					ptr[FLOWSPEC_TYPE_PAYLD_TYPE_OFFSET] |= (uint8_t)val << 4;
					break;
				case FLOW_PAYLD_OTYPE:
					ptr[FLOWSPEC_TYPE_PAYLD_TYPE_OFFSET] |= (uint8_t)val;
					break;
				case FLOW_PAYLD_OFFSET:
					ptr[FLOWSPEC_TYPE_PAYLD_OFFSET_OFFSET] = (uint8_t)(val >> 8);
					ptr[FLOWSPEC_TYPE_PAYLD_OFFSET_OFFSET + 1] = (uint8_t)val;
					break;
				case FLOW_PAYLD_CTLEN:
					ptr[FLOWSPEC_TYPE_PAYLD_CONTL_OFFSET] = (uint8_t)val;
					contlen = ptr[FLOWSPEC_TYPE_PAYLD_CONTL_OFFSET];
					comp = realloc(r->flow.components[type], complen + 2 * contlen);
					if (comp == NULL)
						err(1, NULL);
					memset(comp + complen, 0, 2 * contlen);
					complen += 2 * contlen;
					break;
				default:
					break;
		...
		case FLOW_PAYLD_CTMASK:
			contlen = ptr[FLOWSPEC_TYPE_PAYLD_CONTL_OFFSET];
			offset += FLOWSPEC_TYPE_PAYLD_HL;
			if (contlen <= 0 || complen - offset < 2 * contlen)
				return (0);
			switch (toktype) {
				case FLOW_PAYLD_CONTENT:
					push_numstr(word, comp + offset, contlen);
					break;
				case FLOW_PAYLD_CTMASK:
					push_numstr(word, comp + offset + contlen, contlen);
					comp[0] = complen - 1;
					break;
				default:
					break;
