...
static int bgp_flowspec_payload_call_non_opaque_decode(uint8_t *nlri_content, int len,
					       struct bgp_pbr_match_val_payload *mval,
					       uint8_t *match_num, int *error)
{
	int ret;

	ret = bgp_flowspec_payload_decode(
			     BGP_FLOWSPEC_CONVERT_TO_NON_OPAQUE,
			     nlri_content,
			     len,
			     mval, error);
	if (*error < 0)
		flog_err(EC_BGP_FLOWSPEC_PACKET,
			 "%s: flowspec_payload_decode error %d", __func__, *error);
	else
		*match_num = *error;
	return ret;
}
...
int bgp_flowspec_payload_decode(enum bgp_flowspec_util_nlri_t type,
			   uint8_t *nlri_ptr,
			   uint32_t max_len,
			   void *result, int *error)
{
	uint16_t complen, payld_offset, contlen, offset = 0;
	uint8_t loop = 0;
	char *ptr = (char *)result; /* for return_string */
	int len_string = BGP_FLOWSPEC_STRING_DISPLAY_MAX;
	int len_written;
	...
	*error = 0;
	complen = hexstr2num(&nlri_ptr[offset], 1) + 1;
	++offset;
	if (complen > max_len) {
		*error = -1;
		return complen;
	...
	do {
		payld_offset = ((uint16_t)nlri_ptr[offset + BGP_FLOWSPEC_PAYLD_OFFSET_OFFSET] << 8) | (uint16_t)nlri_ptr[offset + BGP_FLOWSPEC_PAYLD_OFFSET_OFFSET + 1];
		contlen = nlri_ptr[offset + BGP_FLOWSPEC_PAYLD_CONTL_OFFSET];
		switch (type) {
			case BGP_FLOWSPEC_RETURN_STRING:
				if (loop != 0) {
					len_written = snprintf(ptr, len_string, "\n\t&& ");
				...
				len_written = snprintf(ptr, len_string, "ptye %u ", nlri_ptr[offset + BGP_FLOWSPEC_PAYLD_TYPE_OFFSET] >> 4);
				len_string -= len_written;
				ptr += len_written;

				len_written = snprintf(ptr, len_string, "otype %u ", nlri_ptr[offset + BGP_FLOWSPEC_PAYLD_TYPE_OFFSET] & 0xf);
				len_string -= len_written;
				ptr += len_written;
	
				len_written = snprintf(ptr, len_string, "offset %u ", payld_offset);
				len_string -= len_written;
				ptr += len_written;

				len_written = snprintf(ptr, len_string, "length %u ", contlen);
				len_string -= len_written;
				ptr += len_written;
				...
				{
					if (i == 0) {
						len_written = snprintf(ptr, len_string, "content 0x");
						len_string -= len_written;
						ptr += len_written;
					}
					len_written = snprintf(ptr, len_string, "%X", nlri_ptr[offset + BGP_FLOWSPEC_PAYLD_HL + i]);
					len_string -= len_written;
					ptr += len_written;
				}
				...
				{
					if (i == 0) {
						len_written = snprintf(ptr, len_string, " mask 0x");
						len_string -= len_written;
						ptr += len_written;
					}
					len_written = snprintf(ptr, len_string, "%X", nlri_ptr[offset + BGP_FLOWSPEC_PAYLD_HL + contlen + i]);
					len_string -= len_written;
					ptr += len_written;
				}
				break;
			case BGP_FLOWSPEC_CONVERT_TO_NON_OPAQUE:
				mval = realloc((struct bgp_pbr_match_val_payload *)result, (loop + 1) * sizeof(struct bgp_pbr_match_val_payload));
				mval[loop].ptype = nlri_ptr[offset + BGP_FLOWSPEC_PAYLD_TYPE_OFFSET] >> 4;
				mval[loop].otype = nlri_ptr[offset + BGP_FLOWSPEC_PAYLD_TYPE_OFFSET] & 0xf;
				mval[loop].offset = payld_offset;
				mval[loop].contlen = contlen;
				mval[loop].content = malloc(contlen);
				mval[loop].mask = malloc(contlen);
				for (int i = 0; i < contlen; i++)
				{
					mval[loop].content[i] = nlri_ptr[offset + BGP_FLOWSPEC_PAYLD_HL + i];
					mval[loop].mask[i] = nlri_ptr[offset + BGP_FLOWSPEC_PAYLD_HL + contlen + i];
				}
				result = (void *)mval;
				break;
			case BGP_FLOWSPEC_VALIDATE_ONLY:
		...
		offset += BGP_FLOWSPEC_PAYLD_HL + 2 * contlen;
		++loop;
				  ...
				  afi_t afi)
{
	...
	struct bgp_pbr_match_val *mval;
	struct bgp_pbr_match_val_payload *mval_payload;
	uint8_t *match_num;
	uint8_t bitmask = 0;
	int ret = 0, type;
	...
	while (offset < len - 1 && error >= 0) {
		...
		case FLOWSPEC_FRAGMENT:
			...
			break;
		case FLOWSPEC_PAYLOAD:
			match_num = &(bpem->match_payload_num);
			mval_payload = (struct bgp_pbr_match_val_payload *)bpem->payload;
			offset += bgp_flowspec_payload_call_non_opaque_decode(
							nlri_content + offset,
							len - offset,
							mval_payload, match_num,
							&error);
			break;
		default:
