...
static int snprintf_bgp_pbr_match_val_payload(char *str, int len,
				      ...
				      const char *prepend)
{
	...
	ptr += snprintf(ptr, len, "ptype %u ", mval->ptype);
	ptr += snprintf(ptr, len, "otype %u ", mval->otype);
	ptr += snprintf(ptr, len, "offset %u ", mval->offset);
	ptr += snprintf(ptr, len, "length %u ", mval->contlen);
	for (int i = 0; i < mval->contlen; i++) {
		if (i == 0)
			ptr += snprintf(ptr, len, "content 0x");
		ptr += snprintf(ptr, len, "%X", mval->content[i]);
	}
	for (int i = 0; i < mval->contlen; i++) {
		if (i == 0)
			ptr += snprintf(ptr, len, " mask 0x");
		ptr += snprintf(ptr, len, "%X", mval->mask[i]);
	}
...
static int bgp_pbr_validate_policy_route(struct bgp_pbr_entry_main *api)
{
	...
	if (api->match_payload_num > 0) {
		if (api->payload == NULL) {
			if (BGP_DEBUG(pbr, PBR))
				zlog_debug("BGP: match payload operation:length too small. ignoring.");
			return 0;
		}
	}
				     ...
				     struct bgp_pbr_entry_main *api)
{
	...
	if (ret < 0)
		return -1;
	/* extract actions from flowspec ecom list */
	if (path && bgp_attr_get_ecommunity(path->attr)) {
		...
		for (i = 0; i < ecom->size; i++) {
			...
			if ((ecom_eval->val[1] ==
				...
				else if ((api_action->action == ACTION_TRAFFICRATE_PPS)
				    && api->actions[i].u.r.rate_pps == 0)
					discard_action_found = true;
			}
			api->action_num++;
		}
...
uint32_t bgp_pbr_match_hash_key(const void *arg)
{
	...
	key = jhash(&pbm->payload_length, 1, key);
	key = jhash(pbm->payload, pbm->payload_length, key);
...
bool bgp_pbr_match_hash_equal(const void *arg1, const void *arg2)
{
	...
	if (r1->payload_length != r2->payload_length)
		return false;

	for (int i = 0; i < r1->payload_length; i++)
		if (r1->payload[i] != r2->payload[i])
			return false;
	return true;
}
...
void bgp_pbr_print_policy_route(struct bgp_pbr_entry_main *api)
{
	...
	if (api->match_payload_num > 0)
		INCREMENT_DISPLAY(ptr, nb_items, len);
	for (i = 0; i < api->match_payload_num; i++) {
		delta = snprintf_bgp_pbr_match_val_payload(ptr, len, &api->payload[i],
						   i > 0 ? NULL : "@payload: ");
		len -= delta;
		ptr += delta;
	}
	...
	for (i = 0; i < api->action_num; i++) {
		...
		case ACTION_TRAFFICRATE:
			...
			ptr += delta;
			break;
		case ACTION_TRAFFICRATE_PPS:
			INCREMENT_DISPLAY(ptr, nb_items, len);
			delta = snprintf(ptr, len, "@set rate(pps) %u",
					api->actions[i].u.r.rate_pps);
			len -= delta;
			ptr += delta;
			break;
